...
def vp_flip(vp_bin,bins):
    ###          -pi/2 - <-
    ### -pi/pi               0
    ###          pi/2  + <-
    return ((bins/2) - vp_bin)%bins if vp_bin >= 0 else -(((bins/2) - (-vp_bin))%bins)
...
def transform_instance_annotations(
    annotation, transforms, image_size, bins, *, keypoint_hflip_indices=None
):
    """
    ...
    """
    rot = annotation["bbox"][4] if len(annotation["bbox"]) == 5 else -999
    bbox = BoxMode.convert(annotation["bbox"][:4], annotation["bbox_mode"], BoxMode.XYXY_ABS)
    # Note that bbox is 1d (per-instance bounding box)
    annotation["bbox"] = transforms.apply_box([bbox])[0]
    annotation["bbox_mode"] = BoxMode.XYXY_ABS
    if rot != -999: # Check if there is rotation with boxes
        if isinstance(transforms.transforms[-1], T.HFlipTransform):
            rot = rb_flip(rot)
        annotation["bbox"] = np.concatenate((annotation["bbox"],np.array([rot])))
    ...
    # Apply rotation to viewpoint instances
    if isinstance(transforms.transforms[-1], T.HFlipTransform) and "viewpoint" in annotation:
        if len(annotation["viewpoint"]) == 2:
            annotation["viewpoint"] = [vp_flip(annotation["viewpoint"][0],bins),vp_flip(annotation["viewpoint"][1],2*math.pi)]
        else:
            annotation["viewpoint"] = [vp_flip(annotation["viewpoint"][0],bins)]
    if isinstance(transforms.transforms[-1], T.HFlipTransform) and "bbox3D" in annotation:
        annotation["bbox3D"][0] = image_size[1] - annotation["bbox3D"][0]
    return annotation
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if len(annos) and "viewpoint" in annos[0]:
        ...
        if len(annos[0]["viewpoint"]) == 2:
            viewpoints_rads = torch.tensor(viewpoints[:,1], dtype=torch.float32)
